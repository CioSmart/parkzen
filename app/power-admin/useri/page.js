...
export default function PowerAdminUseri() {
  ...
  const [form, setForm] = useState({ nume: '', email: '', password: '', tip: 'admin', companie_id: '' })
  ...
  async function fetchDate() {
    ...
    const [{ data: useriData }, { data: companiiData }] = await Promise.all([
      supabase.from('users').select('*, companii(nume)').order('nume'),
  ...
  function deschideForm(u = null) {
    if (u) {
      ...
      setForm({
        ...
        password: '',
        tip: u.tip || 'normal',
        companie_id: u.companie_id || ''
      })
    } else {
      setEditUser(null)
      setForm({ nume: '', email: '', password: '', tip: 'admin', companie_id: '' })
    }
    setShowForm(true)
    setMsg('')
  }
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={form.companie_id}
                    onChange={e => setForm({ ...form, companie_id: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
