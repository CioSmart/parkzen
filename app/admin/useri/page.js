...
export default function AdminUseri() {
  ...
  async function fetchDate(u) {
    setLoading(true)
    const [{ data: useriData }, { data: locatiiData }] = await Promise.all([
      supabase
        .from('users')
        .select('*')
  ...
  function deschideForm(u = null) {
    if (u) {
      setEditUser(u)
      setForm({ nume: u.nume || '', email: u.email || '', password: '', tip: u.tip || 'normal' })
    } else {
      setEditUser(null)
  ...
  async function toggleAlocare(locatieId) {
    ...
    if (alocariUser.includes(locatieId)) {
      ...
      await supabase
        .from('alocari_locatii')
        .insert({ user_id: showAlocare.id, locatie_id: locatieId })
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-gray-800">👥 Useri companie</h2>
            <button
          ...
          {showAlocare && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
              <h3 className="font-semibold text-blue-800 mb-3">
                📍 Locații alocate pentru {showAlocare.nume || showAlocare.email}
              </h3>
                    ...
                    <tr key={u.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          u.tip === 'admin' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'
                        }`}>
                          {u.tip}
                        </span>
                      </td>
